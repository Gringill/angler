...
public class PathFinder {
    ...
    public PathFinder(Entity entity) {
        finders.add(this);
        this.entity = entity;
        gameplay = entity.getGameplay();
        nodemap = gameplay.getLevel().getTileMap().getNodeMap();
    }
    ...
    private ArrayList<Vector2> findPath() {
        ...
        while (pool_open.size() > 0) {
            ...
            if (current.equals(goal)) {
                buildPath(goal);
                path.set(0, vStart.copy().multiply((Util.getTileSize() / getNodemap().getDensity())));
                path.set(path.size() - 1, vGoal.copy().multiply((Util.getTileSize() / getNodemap().getDensity())));
                Logger.log("Found path successfully . . . Now lets smooth", "PathFinder", false);
                Vector2 cornerAngle;
                for (int i = 0; i < path.size() - 1; i++) {
                    if (!(cornerAngle = nodemap.isCornerPathable(Util.getTileSize(), getEntity().getPathability(), path.get(i))).equals(NodeMap.CORNER_PATHABLE)) {
