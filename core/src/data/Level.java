...
public class Level implements Json.Serializable {
	...
	public TextureAtlas atlas = new TextureAtlas(Gdx.files.local("bin/atlas0.atlas"));
	private DefaultListModel<GameObject> mod_entity = new DefaultListModel<GameObject>();
	private DefaultListModel<GameObject> mod_tile = new DefaultListModel<GameObject>();
	...
	public static Level deserializeLevel(Editor editor, File selectedFile) {
		...
		editor.getGame().setLevel(level); // Called again when this method
											// returns but
		// needed before then.
		level.getTileMap().connectToGame(editor.getGame());
		editor.initializeObjectEditor();
